{{-- admin/clientes/index.blade.php --}}
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
    ...
    onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: { url: '{{ route('admin.clientes.create') }}' } }))">
    ...
    <div class="bg-white p-3 rounded border border-gray-200 mb-4">
        <form action="{{ route('admin.clientes.index') }}" method="GET" class="flex flex-col md:flex-row gap-3">
            <!-- Campo de búsqueda -->
            ...
            <!-- Filtro por estado -->
            <div>
                <select name="activo" id="activo" onchange="this.form.submit()" class="w-full px-2 py-1 border border-gray-300 rounded focus:outline-none focus:ring-1 focus:ring-blue-500 text-sm">
                    <option value="1" {{ request('activo', '1') == '1' ? 'selected' : '' }}>Activos</option>                
                    <option value="2" {{ request('activo') == '2' ? 'selected' : '' }}>Todos</option>                     
        ...
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                ...
                <tr>
                    ...
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $cliente->nombre }} {{ $cliente->apellido }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $cliente->email }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ $cliente->telefono ?? 'N/A' }}</td>
                    <td class="px-6 py-4 text-sm text-gray-900">{{ \Carbon\Carbon::parse($cliente->created_at)->format('d/m/Y') }}</td>
                    ...
                    <td class="px-6 py-4 text-sm space-x-2">
                        <button type="button" onclick="window.dispatchEvent(new CustomEvent('open-modal', { detail: { url: '{{ route('admin.clientes.show', $cliente) }}' } }))"
                        ...
                        <form action="{{ route('admin.clientes.destroy', $cliente) }}" method="POST" class="inline-block" x-data
